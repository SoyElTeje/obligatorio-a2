...
public:
    ...
    Arista *adyacentes(int vertice)
    {
        grafoDependencias->adyacentes(vertice);
    }

    ListaEnteros *queMisionPuedoHacer()
    {
        ListaEnteros *misiones = new ListaEnteros();
...
public:
    ...
    void agregarDependencia(int mision, int dependeDe)
    {
        grafoOT->agregarDependencia(mision, dependeDe);
    }
    ...
    {
        grafoOT->ejecutarMision(ciudadMision);
    }

    ListaEnteros *queMisionPuedoHacer()
    {
        return grafoOT->queMisionPuedoHacer();
    }

    int mejorMisionSiguiente(int origen)
    {
        ListaEnteros *misiones = grafoOT->queMisionPuedoHacer();
        int *costos = grafoMisiones->dijkstra(origen)[0];
        int min = INT_MAX;
        int idMision = -1;
        while (misiones->sig != NULL)
        {
    ...
    string obtenerNombreMisionPorId(int idMision)
    {
        return misiones[idMision];
    }

    Misiones(int cantidadCiudades, int cantidadMisiones)
    {
        grafoMisiones = new GrafoLista(cantidadCiudades, false, true);
        grafoOT = new GrafoOT(cantidadMisiones);
        ciudades = new string[cantidadCiudades + 1];
        misiones = new string[cantidadMisiones + 1];
        int *mapeoMisionCiudad = new int[cantidadMisiones + 1];
    }
